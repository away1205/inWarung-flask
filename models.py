...
class Product(db.Model):
    ...
    id_product = db.Column(db.String, default=lambda: str(uuid.uuid4()), primary_key=True)
    ...
    restocks = db.relationship("Restock", backref="product", cascade="all, delete-orphan")
...
class CategoryProduct(db.Model):
    ...
    id_category = db.Column(db.String, primary_key=True, default=lambda: str(uuid.uuid4()))
    id_user = db.Column(db.String, db.ForeignKey('user.id_user'), nullable=False)
...
class DetailTransaction(db.Model):
    ...
    id_detail_transaction = db.Column(db.String, primary_key=True, default=lambda: str(uuid.uuid4()))
...
class Transaction(db.Model):
    ...
    id_transaction = db.Column(db.String, primary_key=True, default=lambda: str(uuid.uuid4()))
    id_user = db.Column(db.String, db.ForeignKey('user.id_user'), nullable=False)
...
class Restock(db.Model):
    ...
    id_restock = db.Column(db.String, primary_key=True, default=lambda: str(uuid.uuid4()))
    id_product = db.Column(db.String, db.ForeignKey('product.id_product'), nullable=False)
    restock_date = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
